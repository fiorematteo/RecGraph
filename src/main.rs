...
fn main() {
    ...
    let hofp_reverse = utils::create_handle_pos_in_lnz(&graph_struct.nwp, &graph_path, true);

    //optimize graph
    let mut graph_optimizer =
        qgrams::get_optimizer(&graph_struct, &hofp_forward, &hofp_reverse, qgrams_len);
        ...
        0 => {
            for (i, seq) in sequences.iter().enumerate() {
                let (graph_struct, hofp_forward, hofp_reverse) =
                    graph_optimizer.optimize_graph(seq);
